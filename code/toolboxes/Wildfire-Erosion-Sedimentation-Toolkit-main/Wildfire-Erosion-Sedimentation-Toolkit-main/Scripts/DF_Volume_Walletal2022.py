...
ZonalStatisticsAsTable(burn_basin,uid_field, Temp, temp_tbl, "DATA", "MEAN")
...
ZonalStatisticsAsTable(burn_basin,uid_field, conifer_rc, conifer_tbl, "DATA", "MEAN")
arcpy.AddField_management(conifer_tbl, outconifier, "DOUBLE")
arcpy.CalculateField_management(conifer_tbl, outconifier, "!MEAN!*100", "PYTHON", "")
...
ZonalStatisticsAsTable(burn_basin,uid_field, slp_rc, slp_tbl, "DATA", "MEAN")
slpfield='percslp_'+str(slope_thresh)
arcpy.AddField_management(slp_tbl, slpfield, "DOUBLE")
arcpy.CalculateField_management(slp_tbl, slpfield, "!MEAN!*100", "PYTHON", "")
...
ZonalStatisticsAsTable(burn_basin,uid_field, slp_rc, slpA_tbl, "DATA", "SUM")
...
arcpy.PolygonToRaster_conversion(statgoaoi,sthickfield,statgoaoiras_ST,  "CELL_CENTER", "NONE", str(dx_indem))
...
thick="!MEAN!*10"# convert cm to mm
...
# total volume---note redundant but will change in the futre after debugging is fin
...
arcpy.PolygonToRaster_conversion(statgoaoi,permfield,statgo_perm, 
                                 "CELL_CENTER", "NONE", str(dx_indem))

permtbl=os.path.join(out_temp_dir,basename+'perm.dbf')
ZonalStatisticsAsTable(burn_basin, uid_field, statgo_perm,
                          permtbl, "DATA", "MEAN")
...
arcpy.PolygonToRaster_conversion(statgoaoi,OMfield,statgo_OM, 
                                 "CELL_CENTER", "NONE", str(dx_indem))

OMtbl=os.path.join(out_temp_dir,basename+'OrgMatter.dbf')
ZonalStatisticsAsTable(burn_basin, uid_field, statgo_OM,
                          OMtbl, "DATA", "MEAN")
outfieldom='perc_org'
...
ZonalStatisticsAsTable(burn_basin,uid_field, MGO_wtshd, MGO_tbl, "DATA", "MEAN")
...
ZonalStatisticsAsTable(burn_basin,uid_field, CST_wtshd, CS_tbl, "DATA", "MEAN")
...
ZonalStatisticsAsTable(burn_basin,uid_field, runoffaoi, runoff_tbl, "DATA", "MEAN")
...
ZonalStatisticsAsTable(burn_basin,uid_field, inDEM, E_tbl, "DATA", "MEAN")
...
ZonalStatisticsAsTable(burn_basin,uid_field, TWIR, TWI_tbl, "DATA", "MEAN")
