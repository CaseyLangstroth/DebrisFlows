...
#--------------User Inputs----------------------------------
if usegui==False:
    outfolders=['Sev05i2Y','Sev50i5Y','Sev95i25YV3']
    indir1=r'D:\PLI_Analysis\Outputs\a060N\a070'
    indir2t=r'D:\PLI_Analysis\Outputs\a060N\a070'
    ...
    RivNetin=os.path.join(indir2,basename+'_network.shp')#os.path.join(indir1,basename+"_network.shp")
    ...
    RivNetout=[]#os.path.join(indir2,basename+'_network.shp')    
...
def do_scenario1(L_toe,runout,H_max,w,b,h,slope,L_fanbody,dx,alpha,theta,FD,RW_ras,
                 DV_dbf_id,DV_dbf_vol,catchid,cellsize,bypass):#RGID,
    # catchid=sc_id[i]
    cond = 1
    ...
    if fillcont == 1:
      # no downvalley routing
        ...
        if dist_downstream>cellsize:
            ...
            if round(adj_tot_vol,1)>round(Vol_remain,1):
                cum_sed_vol=np.cumsum(sed_vol)
                idx_a=np.asarray(np.where(cum_sed_vol>Vol_remain)[0])
                #idx_a=idx_a[0]
                # first value is wheere we have extra sediment
                idx=idx_a[0]
                # replace last cell with any remaining sediment
                vt=cum_sed_vol[idx]-Vol_remain
                if vt>0:
                    sed_vol[idx]=sed_vol[idx]-vt
                    if len(idx_a)>1:
            ...
            # else:
            #     rivid=RGID[xi,yi]
        else: # downstream routing is only one cell long
            xi=[XSind]
            yi=[YSind]
            sed_vol=[Vol_remain]
            # rivid=[RGID[xi,yi]]
        ...
        if int(adj_tot_vol2)!=int(Vol_remain):
            raise ValueError('violating conservation of mass')
        # sediment routed down the river channel
        #DownValley_Input[xi,yi]=DownValley_Input[xi,yi]+sed_vol
        #print('meow')
        # print('need to adjust for one elment vs multiple')
        
        for j in np.arange(0,len(sed_vol)):
            # print(j)
            if np.isnan(DV_dbf_id[xi[j]][yi[j]][0]) == True:
                 DV_dbf_id[xi[j]][yi[j]] = [catchid]
                 DV_dbf_vol[xi[j]][yi[j]] = [sed_vol[j]]
                 # DV_dbf_rivid[xi[j]][yi[j]] = [rivid[j]]
    ...
    # generate outputs
    if fillcont==1:
        alldvsed=Vol_input+adj_tot_vol2
        
        fpthdwn=FD[xi,yi]
        if len(fpthdwn)>1:
            fdstdwn=fpthdwn[0]-fpthdwn[-1]
        else:
            fdstdwn=cellsize
    else:
        alldvsed=Vol_input
    ...
    return Vol_input, cond,DV_dbf_id,DV_dbf_vol,fdstdwn,z,alldvsed#,DV_dbf_rivid, DownValley_Input
...
cellsize=float(dx_temp.getOutput(0))
...
arcpy.conversion.PolylineToRaster(RivNet, VBwidthfield,VBWidth_ras, 
                                  "MAXIMUM_LENGTH", "NONE", cellsize, "BUILD")
# convert river shp to raster
...
outras = arcpy.sa.ExtractByMask(FL, chan_ras, "INSIDE"); 
...
snapenv=[chan_points,"VERTEX",str(cellsize*2)+' Unknown']# unknown sets it to map units
...
arcpy.env.snapRaster=chan_ras
gidras=os.path.join(out_temp_dir,basename+'_gidras.tif')
arcpy.conversion.PolylineToRaster(RivNetin, "GridID", gidras, "MAXIMUM_LENGTH", "usarea_km2", cellsize, "BUILD")
RGID=arcpy.RasterToNumPyArray(gidras,nodata_to_value=-9999)*1.0
RGID[RGID==-9999]=np.nan

arcpy.env.snapRaster=fdr

#---------------------------Extract variables of interest----------------------
#.......................Get the data from subcatchments........................
scflds = ["sl_Vol","sub_ID"]
try:
...
sc_id=arr_sc[scflds[1]]
VolV=arr_sc[scflds[0]]
...
# Extract the values to arrays
ppSidtemp=arr_pp[ppflds[0]]
ppgidtemp=arr_pp[ppflds[1]]
pptribtemp=arr_pp[ppflds[2]]
ppFDtemp=arr_pp[ppflds[3]]
...
dist_downstrm=np.zeros(R_V.size)
wedgethick=np.zeros(R_V.size)
totdel=np.zeros(R_V.size)
#sub_id_out=np.zeros(R_V.size)*np.nan
#tdist=np.zeros(R_V.size)
# test array initialization
# x_len=25
# y_len=50
# DV_dbf_id = [[np.nan for y in range(0,y_len)] for x in range(0,x_len)]
# DV_dbf_vol = [[np.nan for y in range(0,y_len)] for x in range(0,x_len)]
# debris_id[3][4]
...
DV_dbf_id = [[[np.nan] for y in range(0,FD.shape[1])] for x in range(0,FD.shape[0])]
DV_dbf_vol = [[[np.nan] for y in range(0,FD.shape[1])] for x in range(0,FD.shape[0])]
# DV_dbf_rivid = [[[np.nan] for y in range(0,FD.shape[1])] for x in range(0,FD.shape[0])]
    ...
    if RES==1:
        ...
        if L_overrun[i] > 0: 
            ...
            #Scenario 1: If Runout >> Valley Width
                ...
                Vol_input[i], cond[i],DV_dbf_id,DV_dbf_vol,dist_downstrm[i],wedgethick[i],totdel[i]=do_scenario1(
                    L_toe,runout, H_max,w,b,h,slope,L_fanbody,dx,alpha,theta,FD,
                    RW_ras,DV_dbf_id,DV_dbf_vol,sc_id[i],cellsize,bypass)
                
                #,DV_dbf_rivid can extract later to save on ram,DV_dbf_rivid,RGID,
...
#%%-------------clean up and process down valley routing data------------------ 
try:
    del i
except:
    pass
# iniitalize arrays for data extraction
x=[]
y=[]
dbfid=[]
dbfvol=[]
...
# extract rows with non nan data t
for i in range(0,xlen):
    for j in range(0,ylen):
        if ~np.isnan(DV_dbf_id[i][j][0])==True:
            x.append(i)
            y.append(j)
            dbfid.append(DV_dbf_id[i][j])
            dbfvol.append(DV_dbf_vol[i][j])
            #dbfgid.append(DV_dbf_rivid[i][j])
# del DV_dbf_id,DV_dbf_vol

dbfgid=RGID[x,y]# get river segment ids for cells with data
gidu=np.unique(dbfgid)# get the unique river segment  values
totalvol=np.zeros(gidu.shape)
percsed=[0]*gidu.shape[0]
scsed=[0]*gidu.shape[0]

for j in range(0,len(gidu)):
    gidi=gidu[j]
    # get all cell indices for a given river segment
    gloc=np.where(dbfgid==gidi)
    glocidx=gloc[0][:]
    # print(gidi)
    voltemp=np.zeros(glocidx.shape)#[]
    sctemp=np.zeros(glocidx.shape)#[]
    
    # pull relevent data for a link
    for k in range(0,len(glocidx)):
        # print(k)
        voltemp[k]=dbfvol[glocidx[k]][0]# get all volumes deposited along a link
        sctemp[k]=dbfid[glocidx[k]][0]#get all catchments deposited along a link
    # print(j,'/',sctemp)
    unisc=np.unique(sctemp)# find number of unique subcatchments contributing
    totsed=np.nansum(voltemp)
    if len(unisc)>1:# if multiple subcatchments contribute
        psed=np.zeros(len(unisc))
        scpid=np.zeros(len(unisc))
        for m in range(0,len(unisc)):
            # print(m)
            scloc=np.where(sctemp==unisc[m])
            voltemp2=np.nansum(voltemp[scloc])
            psed[m]=voltemp2/totsed
            scpid[m]=unisc[m]
        # build final output arrays
        totalvol[j]=totsed
        percsed[j]=','.join(str(item) for item in np.round(psed,2))
        scsed[j]=','.join(str(item) for item in scpid)
    else:
        totalvol[j]=totsed
        percsed[j]='1'
        scsed[j]=str(unisc[0])
...
# build final output arrays
outvol=np.zeros(gidV.shape[0])
outscperc=[0]*gidV.shape[0]
outscid=[0]*gidV.shape[0]
outgid=gidV.copy()

for n in range(0,len(gidu)):
    # print (gidu[n])
    idxfinal=np.where(outgid==gidu[n])[0][0]
    outvol[idxfinal]=totalvol[n]
    outscperc[idxfinal]=percsed[n]
    outscid[idxfinal]=scsed[n]

#....................... Get the data from river depostion....................
rivsedflds=[Rwidthfield,Rdepthfield,"Length_m"]
...
r_sed_thick=outvol/(r_w*r_l)
#.............BUILD LEVEES WHERE CHANNEL IS FILLED WITH SEDIMENT...............
...
#%% -------------WRITE OUTPUTS BACK TO SHAPEFILE-------------------------------

# convert rickman runout nan bck to zero
R_V[np.isnan(R_V)]=0
#................. adding data to the sub-catchments...........................
flds=['VolInput','Scenerio','RunoutLen','DstDwnstrm','maxthick','VolDel_Tot']

for fieldname in flds:
    arcpy.AddField_management(subcatch,fieldname, "Double")

# build 2D array to pull data from
outdata = np.vstack((Vol_input,cond,R_V,wedgethick,dist_downstrm,totdel)).T
...
#................. adding data to the river network...........................
#add these in: levee height; final bed agg after levee #



# adding double values back to river network shapefile
...
# adding text values back to river network shapefile
txtflds=["SCID","PercVol"]

for ftxtname in txtflds:
    arcpy.AddField_management(RivNet,ftxtname, "TEXT")

# build 2D array to pull data from
outdata2 = np.vstack((outscid,outscperc)).T

jj=0
for ftxtname in txtflds:
    with arcpy.da.UpdateCursor(RivNet,ftxtname) as cursor: #Add data
        ii = 0
        for row in cursor:
            row[0]=outdata2[ii,jj]
            cursor.updateRow(row)
            ii+=1
    jj+=1
...
NO LONGER NEED THIS SECTIONS WITH CURRENT CODE STRUCTURE
...
ZonalStatisticsAsTable(RivNet,"FID", outputDEM, temp_tbl, "DATA", "SUM")
