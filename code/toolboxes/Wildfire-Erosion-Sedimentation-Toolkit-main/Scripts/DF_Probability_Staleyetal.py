...
if usegui==True:
    ...
    #FIRE EXTENT
    fire_perimeter=arcpy.GetParameterAsText(5)
    #BURN SEVERITY RASTER
    ftype=arcpy.GetParameterAsText(6)
    #dnbr RASTER
    dNBR=arcpy.GetParameterAsText(7)
    #RAINFALL INTENSITY RASTER
    RI=arcpy.GetParameterAsText(8)
    #OUPUT DATA
    basename=arcpy.GetParameterAsText(9)
    out_temp_dir=arcpy.GetParameterAsText(10)
    makenewoutput=arcpy.GetParameterAsText(11)
    #output subcatments option set to [] and it will update the input
    try:
        subcatchout=arcpy.GetParameterAsText(12)
    except:
        subcatout=[]
    slope_thresh=23
...
if not RITF:
    ...
    RI_ZStat = ZonalStatisticsAsTable(subcatch,uid_field, RI, RI_tbl, "NODATA", "MEAN")
    outfieldi15="avg_i15"
...
fields=[outfieldmh,outfielddnbr,outfieldk,outfieldi15]
...
M1_X3 = arr_db[fields[2]]# Soil K factor
Rain_mag=arr_db[fields[3]]/4 # 15-min rainfall magnitude (mm)

#Link function
chi= -3.63+(0.41*M1_X1*Rain_mag)+(0.67*M1_X2*Rain_mag)+(0.7*M1_X3*Rain_mag) 
