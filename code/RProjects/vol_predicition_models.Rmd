...
colznames(gartnervol) = c('Site', 'VolM', 'Source', 'EstVol')
```
...
```{r}
allplot = ggplot(data = na.omit(all), aes(EstVol, VolM, color = Source)) + geom_point() + geom_abline(mapping = NULL, data = NULL, 1, 0) + ylim(0,15000) +ylab('Modeled Initial Volume (m3)') + xlab('Estimated Initial Volume from Imagery (m3)') + ggtitle('Gartner (2014) and Wall (2023) Volume Models') + theme_bw()+ labs(color ='Volume Model')+ expand_limits(x=0,y=0) 
allplot
